...
const useFaceMonitoring = (videoRef) => {
  const detectorRef = useRef(null);
  const animationRef = useRef(null);
  const initializedRef = useRef(false);

  // 🔹 Helper: always enrich with timestamp + mode
  ...
  // 🔹 Behavior trackers (stable across renders)
  ...
  const multiFaceTracker = useRef(
    createBehaviorTracker({
      thresholdDuration: 3000,   // must persist 3s
      cooldownDuration: 5000,   // wait before triggering again
      gracePeriod: 500,
    })
  ).current;
  ...
  useEffect(() => {
    ...
    const startDetection = () => {
      ...
      const DETECTION_INTERVAL = 300; // 300ms (~3 FPS)
        ...
        if (
          ...
          if (time - lastDetectionTime > DETECTION_INTERVAL) {
            ...
            try {
              ...
              // 2️⃣ Multi-Face Handling
              const validFaces = faces.filter(
                (face) => face.box.width > 100 && face.box.height > 100
              );
  ...
  }, [videoRef, noFaceTracker, headTurnTracker]);
